...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame, min_null_share: float=0.35) -> Dict[str, Any]:
    """
    ...
    dups_lst = df.select_dtypes(include=['object', 'string']).columns
    dups_sum = {}
    for i in dups_lst:
        dup_col_ser = df[i].duplicated()
        perc_non_uniq = dup_col_ser.sum() / len(df[i])
        if perc_non_uniq > 0.75:
            dups_sum[i] = True
    flags['has_high_cardinality_categoricals'] = dups_sum

    nums_df = df.select_dtypes(include=['number'])
    nul_dct = {}
    for i in nums_df.columns:
        if (df[i] == 0).sum() / len(df[i]) > min_null_share:
            nul_dct[i] = True

    flags['has_many_zero_values'] = nul_dct
